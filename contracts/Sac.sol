...
contract SAC is ERC721Enumerable, Ownable {
    ...
    uint256 public SACPrice_whitelist = 100000000000000;
    uint256 public SACPrice_public = 100000000000000;
    ...
    function reserveSAC(
        ...
        require(
            _reserveAmount > 0 && _reserveAmount <= SACReserve,
            "Not enough reserve"
        );
    ...
    function mintSAC(uint numberOfTokens) public payable {
        ...
        require(
            numberOfTokens > 0 && numberOfTokens <= maxSACTx,
            "1 pTX allowed"
        );
        require(msg.value == SACPrice_public.mul(numberOfTokens), "Check ETH");
        require(
            max_mints_per_address[msg.sender].add(numberOfTokens) <=
                maxSACPurchase,
            "Max pW reached"
        );
        ...
        for (uint i = 0; i < numberOfTokens; i++) {
            if (totalSupply() < MAX_SAC) {
                ...
                max_mints_per_address[msg.sender] = max_mints_per_address[
                    msg.sender
                ].add(1);
            } else {
                publicSaleIsActive = !publicSaleIsActive;
                payable(msg.sender).transfer(
                    numberOfTokens.sub(i).mul(SACPrice_public)
                );
                break;
            }
        }
    ...
    function updateMerkleRootVIP(
        bytes32 newmerkleRoot
    ) external onlyAuthorized {
        merkleRootVIP = newmerkleRoot;
    }

    function whitelistedMints(
        uint numberOfTokens,
        ...
        require(
            msg.value == SACPrice_whitelist.mul(numberOfTokens),
            "Check ETH"
        );
        require(
            max_mints_per_address[msg.sender].add(numberOfTokens) <=
        ...
        require(
            MerkleProof.verify(
                merkleProof,
                merkleRoot,
                keccak256(abi.encodePacked(user_))
            ),
            "Check proof"
        );
    ...
    function whitelistedVIPMints(
        ...
        for (uint i = 0; i < numberOfTokens; i++) {
            if (totalSupply() <= SACWhitelistReserve) {
                ...
                whitelistSaleIsActive = !whitelistSaleIsActive;
                payable(msg.sender).transfer(
                    numberOfTokens.sub(i).mul(SACPrice_whitelist)
                );
                break;
            }
        }
    ...
    function setMaxSACPurchase(
        uint256 newMaxSACPurchase
    ) public onlyAuthorized {
        maxSACPurchase = newMaxSACPurchase;
    }

    function setMaxSACPurchaseWl(
        uint256 newMaxSACPurchaseWl
    ) public onlyAuthorized {
        maxSACPurchaseWl = newMaxSACPurchaseWl;
    }

    function setMaxSACPurchaseVip(
        uint256 newMaxSACPurchaseVip
    ) public onlyAuthorized {
